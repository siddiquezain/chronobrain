...
- `RivalSelectorConfig` (`app/replay/strategic_rival.py`) — weights for the dynamic strategic-rival score (`w_proximity=0.40`, `w_adjacency=0.25`, `w_gap_trend=0.15`, `w_pace=0.10`, `w_directional=0.10`, sum 1.0) plus `strategic_gap_ceiling_s=10.0` and `immediate_range_s` (defaults to `GateConfig.overtake_detection_gap_threshold_s` so they never drift). A deterministic scoring model over observable position/gap/trend/pace — **not** measured or learned. Only used by the historical replay's `dynamic_rival` path.
- Tick-level switching policy (`RivalSelectorConfig`, `app/replay/field_state.py`) — `switch_margin=0.10` (challenger must beat the incumbent's live relevance by this), `min_dwell_ticks=12` (~3 s hold before any switch), `_TREND_CLAMP_S_PER_LAP=0.5` (tick-level closing rate capped below the switch margin — public data can't resolve it), `_GAP_EMA_WINDOW_S=3.5` / `_TREND_WINDOW_S=6.0` (causal smoothing of the reconstructed gap), `_REL_EMA_WINDOW_S=2.0` (the relevance score itself is causally EMA-smoothed before the switching policy sees it — a strategic rival should not react to sub-2 s score noise), `_EARLY_LAPS_FOR_CONSTANTS=6` (track length + reference lap time taken from laps ≤ 6, so nothing later can shift them — keeps `FieldTimeline.build` causal). All engineered, all `MODEL_ASSUMPTION`.
- `PlannerConfig` live-state coefficients (`planner.py`) — `unrewarded_energy_price_s_per_mj = 0.7`, `low_soc_pace_erosion_s_per_mj = 0.18`, `defensive_push_value_s = 0.3`, `target_range_s = 3.0`. Engineered laptime-equivalents by which the Monte Carlo planner adjusts each mode's prior for the live race state (car in range, own SoC, opportunity strength). They make the planner state-responsive and stop a mode ranking first on a favourable static prior alone; they are **not** measured car dynamics.
- `ReplayEnergyModel` (normalizer.py) — turns a FastF1 lap's throttle/brake trace into a modelled ERS energy path, because **F1 publishes no ERS state of charge**. Every replayed lap carries `energy_is_modeled = True`. It is an **explicit per-lap accounting model** (causal — baselines only ever contain earlier laps):
  - `deployed(lap)  = max_deploy_per_lap_mj (2.6) * throttle_fraction`
  - `recovered(lap) = max_harvest_per_lap_mj (2.3) * brake_fraction + coast_recovery_mj (1.9) * (1 - throttle_fraction)`
  - `net_swing = (recovered - deployed) - (recovered_base - deployed_base)` — measured against a **nominal** lap so a normal lap is ~net-neutral and the state genuinely accumulates
  - `SoC_next = clip(SoC + net_swing + reversion (0.05) * (nominal_soc_mj (4.5) - SoC), floor_mj (0.3), capacity_mj (9.0))`
  - `reversion` is a *small* anti-drift anchor over 50+ laps, **not** a lock (it was 0.12 and pinned the value near-constant — audit fix).
  The API exposes `deployed_this_lap_mj`, `recovered_this_lap_mj`, `net_swing_mj`, `soc_capacity_mj`, all labelled MODELED. The 2024 cars did not run under the 2026 energy budget — this is a `MODEL_ASSUMPTION` layered on real 2024 telemetry (see `docs/historical-replay.md`).
- `_MGU_K_FULL_POWER_THROTTLE = 0.70` (engine.py) — mean throttle fraction at/above which the **modelled** MGU-K peak reaches the `max_ers_k_power_kw` ceiling. `modeled_mgu_k_peak_kw = min(ceiling, ceiling * mean_throttle / 0.70)` — always ≤ the ceiling by construction, so the compliance `MGU-K power ceiling` check is a genuine verification (`pass`/`breach`), not a hardcoded string. A modelled value from the REAL throttle trace, never measured. `MODEL_ASSUMPTION`.
- `RivalSelectorConfig.position_battle_max_gap_s = 2.0` (`strategic_rival.py`) — an adjacent-in-the-order car is a `POSITION_BATTLE` only within this gap; further away (but still adjacent) it is `STRATEGICALLY_RELEVANT`, not a "battle" (audit Issue 10 — previously *any* adjacent car was `POSITION_BATTLE` regardless of gap). `MODEL_ASSUMPTION`. Role classification is now one function, `strategic_rival.classify_role`, used by both the lap-level and tick-level paths.
