...
- Tick-level switching policy (`RivalSelectorConfig`, `app/replay/field_state.py`) — `switch_margin=0.10` (challenger must beat the incumbent's live relevance by this), `min_dwell_ticks=12` (~3 s hold before any switch), `_TREND_CLAMP_S_PER_LAP=0.5` (tick-level closing rate capped below the switch margin — public data can't resolve it), `_GAP_EMA_WINDOW_S=3.5` / `_TREND_WINDOW_S=6.0` (causal smoothing of the reconstructed gap), `_EARLY_LAPS_FOR_CONSTANTS=6` (track length + reference lap time taken from laps ≤ 6, so nothing later can shift them — keeps `FieldTimeline.build` causal). All engineered, all `MODEL_ASSUMPTION`.
